...
@Command(
    ...
    description = ["Scans files/directories for Aerial specs."]
)
...
fun scan(content: List<String>): Result {
    return Result()
}
...
//        println("Writing ${contents.domains.size} features to $componentsFile")
...
//
//        File(output).mkdirs()
//        File(output).resolve(componentsFile).writeText(gson.toJson(contents.domains))
